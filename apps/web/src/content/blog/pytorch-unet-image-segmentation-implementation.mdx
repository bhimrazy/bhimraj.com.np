...
<Figure caption="The post's UNet, computed from its code. Squares scale with spatial size; the number above each is its channel count. Drag the input size or switch the padding to see why the output is smaller than the input.">
  <UNetExplorer />
</Figure>
