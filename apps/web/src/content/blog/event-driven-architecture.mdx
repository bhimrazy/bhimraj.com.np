...
description: "Event-Driven Architecture (EDA) is a software architecture based on a pub/sub model where an event is produced by the producer, stored on the queue and consumed by the consumers as an event stream."
...
image: "/blog/eda/event-driven-architecture.png"
---
...
![Event-Driven Architecture](/blog/eda/event-driven-architecture.png "Event-Driven Architecture")

_Figure: Event-Driven Architecture_
...
Try both models below: pause a consumer to watch the broker hold its backlog, then switch to event streaming to see consumed events stay in the log and be replayed.

<Figure label="Pub/sub vs event streaming" caption="One producer, a broker and two subscribers. In pub/sub, an event is deleted once every subscriber has consumed it. In event streaming, the broker keeps the last 12 events whether or not they were consumed, so a consumer can step back in at an earlier offset.">
  <PubSubSimulator />
</Figure>
...
### R**equest-driven vs Event-driven**
...
![Request Driven vs Event Driven](/blog/eda/request-driven-vs-event-driven.jpg "Request Driven vs Event Driven")

_Figure: Request Driven vs Event Driven_
...
![Request Driven Way vs Event Driven Way](/blog/eda/request-driven-way-vs-event-driven-way.jpg "Request Driven Way vs Event Driven Way")

_Figure: Comparison between the request-response and event-driven approaches demonstrating how event-driven approaches provide less coupling._
...
Here is the three-service example from above, both ways, on the same clock:

<Figure label="Request-driven vs event-driven" caption="A customer asks C for a ride price. Request-driven: C asks A, then B, and the customer waits for the whole chain. Event-driven: A and B publish changes to a stream on their own schedule and C keeps the latest, so it answers straight away. Time is counted in network hops, not measured latency.">
  <RequestVsEvent />
</Figure>
...
- [1]: Event-driven architecture. In Wikipedia. **[https://en.wikipedia.org/wiki/Event-driven_architecture](https://en.wikipedia.org/wiki/Event-driven_architecture)**
- [2]: Event-Driven Architecture. (n.d.). Amazon Web Services, Inc. **[https://aws.amazon.com/event-driven-architecture](https://aws.amazon.com/event-driven-architecture)**
- [3]: Education, I. C. Event-Driven Architecture. IBM. **[https://www.ibm.com/cloud/learn/event-driven-architecture](https://www.ibm.com/cloud/learn/event-driven-architecture)**
