...
<Figure caption="One producer, a broker and two subscribers. In pub/sub, an event is deleted once every subscriber has consumed it. In event streaming, the broker keeps the last 12 events whether or not they were consumed, so a consumer can step back in at an earlier offset.">
  <PubSubSimulator />
</Figure>
...
<Figure caption="A customer asks C for a ride price. Request-driven: C asks A, then B, and the customer waits for the whole chain. Event-driven: A and B publish changes to a stream on their own schedule and C keeps the latest, so it answers straight away. Time is counted in network hops, not measured latency.">
  <RequestVsEvent />
</Figure>
